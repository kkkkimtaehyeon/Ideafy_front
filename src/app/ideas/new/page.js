...
export default function SubmitIdeaPage() {
    ...
    // 폼 제출 핸들러 (GCS Signed URL 방식으로 수정)
    const handleSubmit = async (e) => {
        e.preventDefault();
        setIsSubmitting(true);

        try {
            let uploadedFilesData = []; // { url, description } 목록

            // --- 1단계: 이미지가 있을 경우, Signed URL 요청 ---
            if (images.length > 0) {
                // 1-1. Signed URL 요청 본문 생성
                const fileMetadatas = images.map(img => ({
                    fileName: img.file.name,
                    fileType: img.file.type,
                    description: img.description
                }));

                // 1-2. 서버에 Signed URL 요청
                // (응답 형식은 [{ signedUrl: '...', fileUrl: '...' }, ...]) 라고 가정합니다.
                const signedUrlResponse = await api.post('/ideas/issue-signedurl', fileMetadatas);
                const signedUrlList = signedUrlResponse.data;

                // --- 2단계: GCS로 파일 업로드 (병렬 처리) ---
                const uploadPromises = signedUrlList.map((urlData, index) => {
                    const file = images[index].file;
                    return fetch(urlData.uploadUrl, { // axios 대신 fetch 사용
                        method: 'PUT',
                        headers: {
                ...
                // 3단계에서 사용할 파일 목록 데이터 준비 (최종 GCS URL 사용)
                uploadedFilesData = signedUrlList.map((urlData, index) => ({
                    url: urlData.readUrl, // 업로드용 signedUrl이 아닌, 서버가 알려준 최종 파일 URL
                    description: images[index].description
                }));
            }

            // --- 3단계: 아이디어 데이터와 파일 URL 목록을 서버에 전송 ---
            const ideaPostData = {
                title: formData.title,
                summary: formData.summary,
                category: formData.category.toUpperCase(),
                ...
                techStacks: formData.techStacks,
                alternatives: formData.alternatives,
                keyMetrics: formData.keyMetrics, // keyMetrics 필드 추가
                urls: urls,
                files: uploadedFilesData // GCS에 업로드된 파일 정보 (URL, 설명)
            };

            // FormData 대신 JSON으로 데이터 전송
            const response = await api.post('/ideas', ideaPostData);
                    ...
                    <form className="space-y-16" onSubmit={handleSubmit}>
                        ...
                        <section className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label className="block text-sm font-medium mb-2 text-muted-dark"
                                           htmlFor="keyMetrics">Key Metrics</label>
                                    <textarea
                                        className="form-textarea w-full rounded-lg border-subtle-dark bg-subtle-dark focus:border-primary p-2"
                                        id="keyMetrics"
                                        name="keyMetrics"
